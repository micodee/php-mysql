...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_nohp)){
        $nohp_err = "Please enter the nohp amount.";     
    ...
    if(empty($name_err) && empty($email_err) && empty($nohp_err)){
        ...
        $sql = "UPDATE contact SET name=?, email=?, nohp=? WHERE id=?";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_email, $param_nohp, $param_id);
